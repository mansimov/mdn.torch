-- Taken from https://github.com/koraykv/unsup
--
...
function kmeans(x, k, niter, batchsize, callback, verbose)
   -- args
   ...
   local centroids = x.new(k,ndims):uniform() -- changed to uniform from normal
   for i = 1,k do
        for j = 1,ndims do
            centroids[{i,j}] = math.max(torch.min(x[{{},j}]), centroids[{i,j}])
            centroids[{i,j}] = math.min(torch.max(x[{{},j}]), centroids[{i,j}])
        end
        if ndims > 1 then -- do we need it ?
